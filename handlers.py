...
RESULT_PROMPT = (
    ...
    "Также добавь короткое описание акции. Например, для Google: 'Google — технологический гигант, лидер в поиске и рекламе'."
)
...
async def process_confirmation(message: Message, state: FSMContext):
    ...
    # Получаем случайную акцию, которая существовала в указанный год
    symbol = get_random_stock(start_year=year)
    # Если акция не найдена, сообщаем пользователю
    if not symbol:
        await message.answer(f"К сожалению, в {year} году не было подходящих акций. Пожалуйста, выбери другой год.")
        return
    # Если акция найдена, продолжаем расчёт
    ...
    try:
        ...
        if habit.lower() != 'нет':
            # Если ответ не 'нет', считаем, что вредная привычка есть
            bot_reply += "\n\nУ тебя есть вредные привычки, которые съедают твои деньги. Пора задуматься!"
