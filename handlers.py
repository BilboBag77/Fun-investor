...
async def process_confirmation(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Структурируем сообщение с эмоджи и HTML
        bot_reply = (
            f"<b>💡 {symbol} ({STOCKS.get(symbol, symbol)})</b>\n"
            f"\n"
            f"🚬 Вместо сигарет ты мог бы инвестировать <b>{daily_spend} {currency}</b> в день в <b>{symbol}</b> с <b>{year}</b> года.\n"
            f"💰 Сегодня у тебя было бы: <b>{total_value:,} {currency}</b>!\n"
            f"\n"
            f"🔥 Вместо того чтобы потратить <b>{total_invested:,} {currency}</b> на курение, ты мог бы заработать <b>+{missed_profit:,} {currency}</b>!\n"
            f"📈 Это целых <b>{profit_percent:.1f}%</b> прибыли!\n"
            f"\n"
            f"❗ Не упусти возможность увеличить свой капитал!\n"
            f"\n"
        ...
        bot_reply += "\n\n<i>*Все цифры примерные, расчёт основан только на динамике актива, без учёта валютных колебаний.*</i>"
